...
| Bounded restricted-source transport | ADR 0023 permits one DEV-only operator-captured CDC evidence envelope after both hosted egress paths returned HTTP 403. Raw bytes remain outside GitHub and public interfaces; the isolated runtime independently verifies and privately persists them for steward review. This is not a general or PROD connector. |
