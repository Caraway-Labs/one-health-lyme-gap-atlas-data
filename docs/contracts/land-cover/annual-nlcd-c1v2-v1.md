...
For a bounded local live Tier-A capture, the operator selects credentials through the normal AWS SDK chain, for example by setting `AWS_PROFILE` and `AWS_REGION` in the local shell, then runs `atlas-data source run --definition config/sources/usgs_annual_nlcd_c1v2_2025_48081.yml --tier A --local-live-acquire`. This explicit option is limited to the validated Annual NLCD SourceDefinition and uses the local checkpoint store. No profile name or credential is embedded in source code or the definition. A cloud worker uses its approved workload credential mechanism.
