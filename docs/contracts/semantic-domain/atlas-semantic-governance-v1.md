...
| New exact-source mapping with unchanged measure meaning | New mapping identity | Prove exact source eligibility and reviewed metadata/lineage under #192 before use. A changed source vintage requires a new mapping identity and immutable input revision; the old mapping remains in the historical registry snapshot. |
...
The #192 mapping registry is compared to `tests/fixtures/semantic_governance/source-mappings-v1-baseline.json` in the normal Quality suite. New IDs and changes require an explicit reviewed baseline update with corresponding source/measure/metadata/lineage tests. A same-ID vintage change fails `SOURCE_MAPPING_REVISION_REQUIRED`. The baseline fixture is an immutable contract snapshot, not a live approval ledger. `validate_historical_release_adapter` rejects the one-source incidence-floor adaptation and any #190 v1 county adaptation of state-unallocated records.
