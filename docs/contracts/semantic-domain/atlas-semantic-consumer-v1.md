...
The paging and canonical-JSON helpers check the same closed consumer field shape before accepting a caller-supplied payload. Extra physical-storage fields and nested object values fail closed even when a field name does not match a restricted-word pattern. The checked-in JSON Schema remains the distributable machine contract; #190–#193 validation remains the scientific and authority gate for newly projected traces.
