...
| Behavioral and adversarial tests | `test_semantic_consumer.py`: county/site/source-only, valid/unknown IDs, bounded filters/pages, states, unsafe values, extra storage-field injection, restricted proof exclusion, reproducible revisions, internal derived exclusion | Synthetic fixtures, not live source approval |
