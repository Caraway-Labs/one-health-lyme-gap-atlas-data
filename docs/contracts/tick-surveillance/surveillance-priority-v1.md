...
Each cohort fixes one construct, source family/product, exact dataset and source version, native grain, and time semantics. County cohorts additionally fix publisher scope version, canonical eligible universe version, cumulative-through date, and taxon/pathogen target. Site/event collection cohorts fix source event date and collection method; effort cohorts also fix effort unit. Individual testing cohorts fix tested date, pathogen target, and individual testing scope. The cohort ID is a hash of that exact context. Missing required context yields `NOT_DEFENSIBLE`, not an inferred cohort. An `UNKNOWN` result with absent event identity remains `EVIDENCE_VERIFICATION` with no cohort or tie group: the verification action is explicit, but no comparison is defensible.
