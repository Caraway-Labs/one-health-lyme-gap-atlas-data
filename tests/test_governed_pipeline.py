...
def test_migrations_are_environment_neutral_and_reject_poc() -> None:
    ...
    assert "V086" in {item["version"] for item in prod_plan}
    assert "V087" in {item["version"] for item in prod_plan}
    assert "V088" in {item["version"] for item in prod_plan}
    assert "V089" in {item["version"] for item in prod_plan}
    assert "V090" in {item["version"] for item in prod_plan}
    assert "V091" in {item["version"] for item in prod_plan}
    ...
    assert "V096" in {item["version"] for item in prod_plan}
    assert "V097" in {item["version"] for item in prod_plan}
    assert "V098" in {item["version"] for item in prod_plan}
    assert "V099" in {item["version"] for item in prod_plan}
    ...
    assert "V083" not in {item["version"] for item in prod_plan}
    assert "V084" not in {item["version"] for item in prod_plan}
    assert "V085" not in {item["version"] for item in prod_plan}
    ...
    assert "V102" in {item["version"] for item in dev_plan}
...
def test_dev_pmc_budget_finalization_is_append_only_and_fail_closed() -> None:
    ...
    assert "SP_FINALIZE_KG_LLM_BUDGET" not in repair.source
    ...
    assert "GRANT SELECT, INSERT ON TABLE GOVERNANCE.LLM_BUDGET_FINALIZATIONS" in repair.source
    assert "OH_LYME_DEV_PIPELINE_RUNTIME" in repair.source
    assert "OH_LYME_DEV_KG_LLM_BUDGET_OWNER" in repair.source
...
def test_dev_pmc_budget_finalization_procedures_keep_budget_owner() -> None:
    repair = next(item for item in load_migrations() if item.version == "V062")
    assert migration_execution_role(repair, DEV_DATABASE) == "OH_LYME_DEV_KG_LLM_BUDGET_OWNER"
    with pytest.raises(ValueError, match="DEV-only"):
        render_migration(repair, "ONE_HEALTH_LYME_GAP_ATLAS_PROD")
    assert "V062" not in {
...
def test_dev_workflow_diagnoses_migration_ddl_without_emitting_query_text() -> None:
    ...
    assert "(CREATE|ALTER|GRANT|REVOKE|USE)" in workflow
    assert "(?is)" not in workflow
    assert "UPPER(QUERY_TEXT)" in workflow
