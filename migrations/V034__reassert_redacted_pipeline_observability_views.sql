...
SELECT
  ...
  COUNT_IF(status = 'IN_PROGRESS' AND lease_expires_at <= CURRENT_TIMESTAMP()) AS expired_lease_artifacts,
...
SELECT r.artifact_id, q.redacted_request:catalog_id::VARCHAR AS catalog,
       q.redacted_request:term::VARCHAR AS matched_term, a.created_at AS captured_at,
       a.byte_count, r.status, r.attempt_count, r.started_at, r.lease_expires_at,
       IFF(r.status = 'IN_PROGRESS' AND r.lease_expires_at <= CURRENT_TIMESTAMP(), TRUE, FALSE) AS has_expired_lease,
       r.redacted_error
...
FROM GOVERNANCE.INGESTION_RUNS WHERE resource_key = 'catalog_discovery';

CREATE OR REPLACE VIEW GOVERNANCE.V_PIPELINE_CATALOG_COVERAGE AS
SELECT q.redacted_request:catalog_id::VARCHAR AS catalog, COUNT(*) AS captured_artifacts,
       COUNT(DISTINCT q.redacted_request:term::VARCHAR) AS distinct_terms, SUM(a.byte_count) AS captured_bytes,
...
FROM GOVERNANCE.RAW_ARTIFACTS a JOIN GOVERNANCE.INGESTION_REQUESTS q ON q.ingestion_request_id = a.ingestion_request_id
...
SELECT (SELECT COUNT(*) FROM GOVERNANCE.CATALOG_DATASETS) AS unique_catalog_datasets,
       ...
       (SELECT COUNT(*) FROM GOVERNANCE.CATALOG_DISCOVERY_REGISTRATIONS WHERE status = 'COMPLETED') AS completed_artifacts,
...
SELECT COALESCE(status, 'NO_SOURCE_VERSION') AS source_version_status, COUNT(*) AS source_versions
FROM GOVERNANCE.DATA_SOURCE_VERSIONS GROUP BY status;

GRANT SELECT ON VIEW GOVERNANCE.V_PIPELINE_OBSERVABILITY_OVERVIEW TO ROLE OH_LYME_{{ ENV }}_STREAMLIT_OWNER;
GRANT SELECT ON VIEW GOVERNANCE.V_PIPELINE_ARTIFACT_BACKLOG TO ROLE OH_LYME_{{ ENV }}_STREAMLIT_OWNER;
GRANT SELECT ON VIEW GOVERNANCE.V_PIPELINE_DISCOVERY_RUNS TO ROLE OH_LYME_{{ ENV }}_STREAMLIT_OWNER;
GRANT SELECT ON VIEW GOVERNANCE.V_PIPELINE_CATALOG_COVERAGE TO ROLE OH_LYME_{{ ENV }}_STREAMLIT_OWNER;
GRANT SELECT ON VIEW GOVERNANCE.V_PIPELINE_REGISTRATION_OUTCOMES TO ROLE OH_LYME_{{ ENV }}_STREAMLIT_OWNER;
GRANT SELECT ON VIEW GOVERNANCE.V_PIPELINE_SOURCE_GOVERNANCE TO ROLE OH_LYME_{{ ENV }}_STREAMLIT_OWNER;
...
GRANT SELECT ON TABLE GOVERNANCE.CATALOG_DISCOVERY_REGISTRATIONS TO ROLE OH_LYME_{{ ENV }}_STREAMLIT_OWNER;
GRANT SELECT ON TABLE GOVERNANCE.CATALOG_RESOURCES TO ROLE OH_LYME_{{ ENV }}_STREAMLIT_OWNER;
GRANT SELECT ON TABLE GOVERNANCE.CATALOG_DISCOVERY_OBSERVATIONS TO ROLE OH_LYME_{{ ENV }}_STREAMLIT_OWNER;
