-- Epic #252: V091 retains the VARIANT write in SELECT form; bind the SQL
-- Scripting parameter explicitly so Snowflake does not resolve it as a column.
...
$$
DECLARE invalid_resource EXCEPTION (-20121, 'Only the two reviewed restricted CDC sources are accepted'); invalid_input EXCEPTION (-20122, 'A supported approval and complete review metadata are required'); incomplete_evidence EXCEPTION (-20123, 'Restricted-source approval requires completed private evidence'); unauthorized EXCEPTION (-20124, 'Reviewer is not an active global steward'); evidence_count NUMBER; steward_count NUMBER; decision_id VARCHAR DEFAULT UUID_STRING(); source_version_id VARCHAR DEFAULT UUID_STRING();
BEGIN
