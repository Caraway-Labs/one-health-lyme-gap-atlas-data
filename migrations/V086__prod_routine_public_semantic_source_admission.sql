-- Epic #252: production admission for the two reviewed public contextual
-- sources. Restricted CDC workbook sources remain outside this procedure.
...
$$
DECLARE
  invalid_resource EXCEPTION (-20007, 'This PROD console supports only reviewed Lyme, SVI, and RUCC sources');
  ...
  unauthorized_steward EXCEPTION (-20005, 'Viewer is not an active steward for this resource');
...
BEGIN
  ...
  IF (RESOURCE_KEY IS NULL OR RESOURCE_KEY NOT IN (
    ...
    'cdc_atsdr_svi_2022_county', 'usda_ers_rucc_2023'
  )) THEN RAISE invalid_resource; END IF;
  ...
  WHERE s.username = :REVIEWER_USERNAME AND s.is_active = TRUE
    AND s.authorization_scope IN ('GLOBAL', :RESOURCE_KEY, 'RESOURCE:' || :RESOURCE_KEY,
      'CATALOG:' || d.catalog_name, 'DOMAIN:cdc.gov', 'DOMAIN:usda.gov');
  ...
  IF (DECISION IN ('APPROVED', 'APPROVED_WITH_CONDITIONS')) THEN
    ...
    WHERE r.resource_key = :RESOURCE_KEY AND r.is_active = TRUE
      AND ((r.resource_key = 'cdc_lyme_x5j9_wybp' AND r.api_dataset_id = 'x5j9-wybp')
        OR (r.resource_key = 'cdc_lyme_qtbi_xd4i' AND r.api_dataset_id = 'qtbi-xd4i')
        OR (r.resource_key = 'cdc_atsdr_svi_2022_county' AND r.api_dataset_id = 'atsdr-svi-2022-county-layer')
        OR (r.resource_key = 'usda_ers_rucc_2023' AND r.api_dataset_id = 'rural-urban-continuum-codes-2023'))
      ...
      AND EXISTS (
        ...
        WHERE p.resource_key = r.resource_key AND p.effective_to IS NULL
          AND ((r.resource_key LIKE 'cdc_lyme_%' AND p.connector_name = 'SOCRATA_SODA2')
            OR (r.resource_key = 'cdc_atsdr_svi_2022_county' AND p.connector_name = 'http_json')
            OR (r.resource_key = 'usda_ers_rucc_2023' AND p.connector_name = 'http_csv'))
      )
