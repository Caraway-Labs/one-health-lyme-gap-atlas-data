-- DEV-only extension for the evidence-only CDC Ixodes county-status candidate.
...
WITH latest_resource AS (
  ...
  WHERE r.is_active = TRUE AND r.resource_key IN (
...
SELECT r.resource_key, r.resource_type, r.canonical_source_url, r.api_dataset_id,
       ...
       IFF(COALESCE(e.material_schema_change_count, 0) > 0
           OR COALESCE(doc.material_document_change_count, 0) > 0, TRUE, FALSE)
...
SELECT r.resource_key, r.resource_type, r.canonical_source_url, r.api_dataset_id,
       ...
       EXISTS (SELECT 1 FROM GOVERNANCE.SOURCE_DOCUMENT_SNAPSHOTS changed_doc
               WHERE changed_doc.resource_key = r.resource_key
                 AND changed_doc.is_material_change) AS has_material_document_change,
       IFF(COALESCE(e.material_schema_change_count, 0) > 0 OR EXISTS (
             SELECT 1 FROM GOVERNANCE.SOURCE_DOCUMENT_SNAPSHOTS changed_doc
             WHERE changed_doc.resource_key = r.resource_key
               AND changed_doc.is_material_change), TRUE, FALSE) AS has_blocking_issue,
       ...
       s.schema_evidence,
       CASE WHEN r.resource_key = 'cdc_tick_ixodes_county_status' THEN
         'Cumulative county tick-surveillance status through 2025-12-31. No records is not evidence of absence. Reported or established status is not abundance, pathogen prevalence, human infection incidence, or individual risk.'
       ELSE
         'County-of-residence human surveillance; use the source-specific reporting era. Do not compare eras without reviewed methodology. Non-geographic line-list data cannot join county contextual facts.'
...
WITH latest_version AS (
  SELECT * FROM GOVERNANCE.DATA_SOURCE_VERSIONS
  WHERE resource_key IN (
    'cdc_lyme_x5j9_wybp', 'cdc_lyme_qtbi_xd4i', 'cdc_tick_ixodes_county_status')
  QUALIFY ROW_NUMBER() OVER (PARTITION BY resource_key ORDER BY created_at DESC) = 1
), latest_run AS (
  SELECT * FROM GOVERNANCE.INGESTION_RUNS
  WHERE resource_key IN (
    'cdc_lyme_x5j9_wybp', 'cdc_lyme_qtbi_xd4i', 'cdc_tick_ixodes_county_status')
...
SELECT seed.resource_key, v.data_source_version_id,
       ...
       v.retired_at, r.status AS latest_ingestion_status,
...
FROM (
  SELECT 'cdc_lyme_x5j9_wybp' AS resource_key
  UNION ALL SELECT 'cdc_lyme_qtbi_xd4i'
  UNION ALL SELECT 'cdc_tick_ixodes_county_status'
) seed
...
GRANT SELECT ON VIEW GOVERNANCE.V_SOURCE_APPROVAL_DETAIL
  TO ROLE OH_LYME_{{ ENV }}_STREAMLIT_OWNER;
GRANT SELECT ON VIEW GOVERNANCE.V_SOURCE_REVIEW_HISTORY
  TO ROLE OH_LYME_{{ ENV }}_STREAMLIT_OWNER;
GRANT SELECT ON VIEW GOVERNANCE.V_SOURCE_PIPELINE_STATUS
  TO ROLE OH_LYME_{{ ENV }}_STREAMLIT_OWNER;
