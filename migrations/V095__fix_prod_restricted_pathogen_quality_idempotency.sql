-- V094 used a non-existent ledger column in its idempotency checks.  Preserve
-- the applied migration checksum and replace only the owner-rights procedure.
...
BEGIN
  ...
  WHERE NOT EXISTS (SELECT 1 FROM GOVERNANCE.DATA_QUALITY_RESULTS WHERE ingestion_run_id=:INGESTION_RUN_ID AND rule_id='restricted_pathogen_source_rows');
  INSERT INTO GOVERNANCE.DATA_QUALITY_RESULTS
  ...
  WHERE NOT EXISTS (SELECT 1 FROM GOVERNANCE.DATA_QUALITY_RESULTS WHERE ingestion_run_id=:INGESTION_RUN_ID AND rule_id='restricted_pathogen_unique_fips');
  INSERT INTO GOVERNANCE.DATA_QUALITY_RESULTS
  ...
  WHERE NOT EXISTS (SELECT 1 FROM GOVERNANCE.DATA_QUALITY_RESULTS WHERE ingestion_run_id=:INGESTION_RUN_ID AND rule_id='restricted_pathogen_status_values');
