...
-- A review queue contains work still awaiting a steward decision. Decision
-- history remains available through V_SOURCE_REVIEW_HISTORY.
...
WHERE r.is_active = TRUE
  AND ld.manual_review_decision_id IS NULL;

-- CREATE OR REPLACE VIEW removes direct grants, so restore the owner-rights
-- Streamlit application's read access in the same migration.
