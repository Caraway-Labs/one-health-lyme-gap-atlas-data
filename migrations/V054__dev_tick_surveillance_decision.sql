-- DEV-only decision boundary for the evidence-only CDC Ixodes candidate.
...
$$
DECLARE
  invalid_resource EXCEPTION (-20007, 'This DEV console supports only the three allowlisted CDC sources');
...
BEGIN
  BEGIN TRANSACTION;
  IF (RESOURCE_KEY IS NULL OR RESOURCE_KEY NOT IN (
      'cdc_lyme_x5j9_wybp', 'cdc_lyme_qtbi_xd4i', 'cdc_tick_ixodes_county_status'))
    THEN RAISE invalid_resource;
  END IF;
  IF (DECISION NOT IN (
      'APPROVED', 'APPROVED_WITH_CONDITIONS', 'REJECTED', 'RETIRED', 'DEFERRED'))
    THEN RAISE invalid_decision;
  END IF;
  IF (RATIONALE IS NULL OR LENGTH(TRIM(RATIONALE)) < 10 OR LENGTH(RATIONALE) > 10000)
    THEN RAISE invalid_rationale;
  END IF;
  IF (DECISION IN ('APPROVED_WITH_CONDITIONS', 'REJECTED', 'RETIRED', 'DEFERRED')
      AND (CONDITIONS IS NULL OR NOT IS_ARRAY(CONDITIONS) OR ARRAY_SIZE(CONDITIONS) = 0))
    THEN RAISE missing_conditions;
  END IF;
  IF (NULLIF(TRIM(REVIEWER_USERNAME), '') IS NULL
      OR NULLIF(TRIM(APP_VERSION), '') IS NULL
      OR NULLIF(TRIM(CORRELATION_ID), '') IS NULL)
    THEN RAISE invalid_request;
  END IF;

  SELECT COUNT(*) INTO :steward_count FROM GOVERNANCE.APPROVAL_STEWARDS s
  JOIN GOVERNANCE.CATALOG_RESOURCES r
    ON r.resource_key = :RESOURCE_KEY AND r.is_active = TRUE
  ...
  IF (DECISION IN ('APPROVED', 'APPROVED_WITH_CONDITIONS')) THEN
    ...
    WHERE r.resource_key = :RESOURCE_KEY AND r.is_active = TRUE
      AND (
        (r.resource_key = 'cdc_lyme_x5j9_wybp' AND r.api_dataset_id = 'x5j9-wybp')
        OR (r.resource_key = 'cdc_lyme_qtbi_xd4i' AND r.api_dataset_id = 'qtbi-xd4i')
        OR (r.resource_key = 'cdc_tick_ixodes_county_status'
            AND r.api_dataset_id = 'cdc-ixodes-county-status-2025')
      )
      AND d.metadata_payload IS NOT NULL
      AND EXISTS (
        SELECT 1 FROM GOVERNANCE.SOURCE_ACCESS_PROFILES p
        WHERE p.resource_key = r.resource_key AND p.effective_to IS NULL
          AND ((r.resource_key LIKE 'cdc_lyme_%' AND p.connector_name = 'SOCRATA_SODA2')
            OR (r.resource_key = 'cdc_tick_ixodes_county_status'
                AND p.connector_name = 'HTTP_XLSX_V1')))
      AND EXISTS (
        SELECT 1 FROM GOVERNANCE.SOURCE_DOCUMENT_SNAPSHOTS doc
        WHERE doc.resource_key = r.resource_key)
      AND EXISTS (
        SELECT 1 FROM GOVERNANCE.SCHEMA_SNAPSHOTS schema
        WHERE schema.resource_key = r.resource_key)
      AND EXISTS (
        SELECT 1 FROM GOVERNANCE.DATASET_QUALITY_ASSESSMENTS a
    ...
    SELECT :blocking_count + COUNT(*) INTO :blocking_count
    FROM GOVERNANCE.SOURCE_DOCUMENT_SNAPSHOTS doc
    ...
    SELECT :source_version_id, :RESOURCE_KEY,
           IFF(:DECISION = 'APPROVED', 'APPROVED', 'CONDITIONAL'),
           :decision_id, CURRENT_TIMESTAMP();
  END IF;

  IF (active_source_version_id IS NOT NULL) THEN
    UPDATE GOVERNANCE.DATA_SOURCE_VERSIONS
  ...
  COMMIT;
  RETURN OBJECT_CONSTRUCT('decision_id', :decision_id,
