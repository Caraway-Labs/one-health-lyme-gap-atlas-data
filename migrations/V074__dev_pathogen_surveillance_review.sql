...
BEGIN
  ...
  WHERE s.username = :REVIEWER_USERNAME AND s.is_active = TRUE
    AND s.authorization_scope IN ('GLOBAL', :RESOURCE_KEY, 'RESOURCE:' || :RESOURCE_KEY, 'CATALOG:' || d.catalog_name, 'DOMAIN:cdc.gov');
  ...
  IF (DECISION IN ('APPROVED', 'APPROVED_WITH_CONDITIONS')) THEN
    ...
    WHERE r.resource_key = :RESOURCE_KEY AND r.is_active = TRUE
      ...
      AND EXISTS (SELECT 1 FROM GOVERNANCE.SOURCE_ACCESS_PROFILES p WHERE p.resource_key = r.resource_key AND p.effective_to IS NULL
        AND ((r.resource_key LIKE 'cdc_lyme_%' AND p.connector_name = 'SOCRATA_SODA2') OR (r.resource_key IN ('cdc_tick_ixodes_county_status', 'cdc_tick_ixodes_pathogen_status') AND p.connector_name = 'HTTP_XLSX_V1')))
