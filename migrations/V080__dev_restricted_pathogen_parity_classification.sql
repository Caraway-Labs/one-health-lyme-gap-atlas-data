-- DEV-only immutable parity classification for restricted pathogen source coverage.
...
BEGIN
  ...
  IF (NOT EXISTS (SELECT 1 FROM GOVERNANCE.INGESTION_RUNS WHERE ingestion_run_id=:INGESTION_RUN_ID AND resource_key='cdc_tick_ixodes_pathogen_status' AND status='COMPLETED')) THEN RAISE invalid_run; END IF;
