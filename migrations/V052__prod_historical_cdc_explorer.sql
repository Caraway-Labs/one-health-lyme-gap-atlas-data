...
-- PROD only. This migration runs as OH_LYME_PROD_GOVERNED_VIEW_OWNER,
-- replacing only the governed views that role already owns.
CREATE OR REPLACE VIEW GOVERNANCE.V_SOURCE_INGESTION_VALIDATION COPY GRANTS AS
WITH raw_rollup AS (
  ...
  FROM (SELECT data_source_version_id, ingestion_run_id, artifact_id, retrieved_at FROM RAW.CDC_LYME_X5J9_WYBP
    UNION ALL SELECT data_source_version_id, ingestion_run_id, artifact_id, retrieved_at FROM RAW.CDC_LYME_QTBI_XD4I)
  ...
  FROM (SELECT * FROM CONFORMED.CONFORMED_CDC_LYME_X5J9_WYBP
    UNION ALL SELECT * FROM CONFORMED.CONFORMED_CDC_LYME_QTBI_XD4I)
  GROUP BY data_source_version_id
), quality_rollup AS (
  SELECT ingestion_run_id, COUNT(*) AS quality_result_count,
         COUNT_IF(status = 'FAILED') AS failed_quality_result_count
  FROM GOVERNANCE.DATA_QUALITY_RESULTS GROUP BY ingestion_run_id
)
SELECT version.data_source_version_id, version.resource_key, version.status AS source_version_status,
       ...
       IFF(version.resource_key='cdc_lyme_qtbi_xd4i',
        '2008-2021 surveillance era; not comparable to 2022 onward without reviewed methodology',
...
WHERE version.resource_key IN ('cdc_lyme_x5j9_wybp','cdc_lyme_qtbi_xd4i');

CREATE OR REPLACE VIEW GOVERNANCE.V_DATA_EXPLORER_SOURCE_VERSIONS COPY GRANTS AS
...
CREATE OR REPLACE VIEW GOVERNANCE.V_DATA_EXPLORER_CONFORMED_CDC COPY GRANTS AS
...
CREATE OR REPLACE VIEW GOVERNANCE.V_DATA_EXPLORER_ANALYTICS_CDC COPY GRANTS AS
-- No reviewed aggregate exists. Preserve CONFORMED grain and provenance.
SELECT source_record_id, data_source_version_id, ingestion_run_id, artifact_id,
       county_fips, report_year, case_status, sex, age_category_years, frequency,
       source_value_status, geography_semantics, source_resolution, temporal_window,
       caveat, retrieved_at
